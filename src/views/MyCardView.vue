<template>
  <div class="mt-28 container-wrapper">
    <h2>My card</h2>

    <div class="gap-6 grid-cols-3 grid mt-4">
      <div class="col-span-2">
        <div class="border rounded-2xl p-3 px-4">
          <div class="flex gap-2 items-end cardMeal">
            <h3>Menu</h3>
            <p class="text-base text-gray-400">4 meals</p>
          </div>
          <div class="flex w-full item-center justify-between py-4 border-b">
            <div class="flex gap-3 text-start">
              <img
                class="w-20 h-20 rounded-xl object-cover object-center"
                src="https://static.vecteezy.com/system/resources/thumbnails/030/201/658/small/beautiful-natural-landscape-of-green-meadows-rivers-and-mountains-free-photo.jpg"
                alt=""
              />
              <div class="">
                <p class="text-sm font-semibold">Chicken & Ribs Combo</p>
                <p class="p-0">
                  Lorem ipsum dolor sit amet consectetur
                  <br class="hidden lg:block" />
                  adipisicing elit. Soluta dicta dolorum, pariatur
                </p>
              </div>
            </div>

            <div class="flex items-center space-x-6">
              <div class="space-x-2.5 mr-6">
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-minus" />
                </button>
                <span class="text-bold">2</span>
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-plus" />
                </button>
              </div>
              <span class="text-lg font-semibold">$12.40</span>
              <v-icon
                name="bi-trash"
                class="cursor-pointer hover:text-rose-400 ease-in duration-100 text-gray-500"
              />
            </div>
          </div>
          <div class="flex w-full item-center justify-between py-4 border-b">
            <div class="flex gap-3 text-start">
              <img
                class="w-20 h-20 rounded-xl object-cover object-center"
                src="https://static.vecteezy.com/system/resources/thumbnails/030/201/658/small/beautiful-natural-landscape-of-green-meadows-rivers-and-mountains-free-photo.jpg"
                alt=""
              />
              <div class="">
                <p class="text-sm font-semibold">Chicken & Ribs Combo</p>
                <p class="p-0">
                  Lorem ipsum dolor sit amet consectetur
                  <br class="hidden lg:block" />
                  adipisicing elit. Soluta dicta dolorum, pariatur
                </p>
              </div>
            </div>

            <div class="flex items-center space-x-6">
              <div class="space-x-2.5 mr-6">
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-minus" />
                </button>
                <span class="text-bold">2</span>
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-plus" />
                </button>
              </div>
              <span class="text-lg font-semibold">$12.40</span>
              <v-icon
                name="bi-trash"
                class="cursor-pointer hover:text-rose-400 ease-in duration-100 text-gray-500"
              />
            </div>
          </div>
          <div class="flex w-full item-center justify-between py-4 border-b">
            <div class="flex gap-3 text-start">
              <img
                class="w-20 h-20 rounded-xl object-cover object-center"
                src="https://static.vecteezy.com/system/resources/thumbnails/030/201/658/small/beautiful-natural-landscape-of-green-meadows-rivers-and-mountains-free-photo.jpg"
                alt=""
              />
              <div class="">
                <p class="text-sm font-semibold">Chicken & Ribs Combo</p>
                <p class="p-0">
                  Lorem ipsum dolor sit amet consectetur
                  <br class="hidden lg:block" />
                  adipisicing elit. Soluta dicta dolorum, pariatur
                </p>
              </div>
            </div>

            <div class="flex items-center space-x-6">
              <div class="space-x-2.5 mr-6">
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-minus" />
                </button>
                <span class="text-bold">2</span>
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-plus" />
                </button>
              </div>
              <span class="text-lg font-semibold">$12.40</span>
              <v-icon
                name="bi-trash"
                class="cursor-pointer hover:text-rose-400 ease-in duration-100 text-gray-500"
              />
            </div>
          </div>
          <div class="flex w-full item-center justify-between py-4 border-b">
            <div class="flex gap-3 text-start">
              <img
                class="w-20 h-20 rounded-xl object-cover object-center"
                src="https://static.vecteezy.com/system/resources/thumbnails/030/201/658/small/beautiful-natural-landscape-of-green-meadows-rivers-and-mountains-free-photo.jpg"
                alt=""
              />
              <div class="">
                <p class="text-sm font-semibold">Chicken & Ribs Combo</p>
                <p class="p-0">
                  Lorem ipsum dolor sit amet consectetur
                  <br class="hidden lg:block" />
                  adipisicing elit. Soluta dicta dolorum, pariatur
                </p>
              </div>
            </div>

            <div class="flex items-center space-x-6">
              <div class="space-x-2.5 mr-6">
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-minus" />
                </button>
                <span class="text-bold">2</span>
                <button class="w-7 h-7 text-center rounded-full bg-slate-100">
                  <v-icon name="hi-solid-plus" />
                </button>
              </div>
              <span class="text-lg font-semibold">$12.40</span>
              <v-icon
                name="bi-trash"
                class="cursor-pointer hover:text-rose-400 ease-in duration-100 text-gray-500"
              />
            </div>
          </div>
        </div>

        <div class="flex gap-2 text-blue-500 text-center mt-5">
          <v-icon name="md-keyboardbackspace-outlined" />
          <a class="font-semibold" href="">Add more meals</a>
        </div>
      </div>

      <div>
        <div class="border rounded-2xl p-3 px-4">
          <div class="flex rounded-3xl bg-slate-200 p-1">
            <button class="py-4 rounded-3xl w-1/2 text-lg font-medium">
              Delivery
            </button>
            <button
              class="py-4 bg-gray-800 rounded-3xl text-lg text-white w-1/2 font-medium"
            >
              Pick up
            </button>
          </div>
          <h4 class="text-base text-gray-400 font-normal mt-4">
            Restaurant address
          </h4>
          <div class="w-full h-56 rounded-2xl border mt-2" />
          <p class="mt-2 p-0 font-semibold text-md">San Diego</p>
          <p class="text-sm text-gray-400">Uzbekistan tashkent, UZB</p>
          <span class="text-md text-gray-500">Lorem ipsum dolor sit amet.</span>
        </div>
        <div class="border rounded-2xl p-3 px-4 mt-8">
          <h3 class="text-gray-700">Payment summary</h3>
          <div class="space-y-2">
            <div class="flex justify-between">
              Subtotal <span class="text-gray-600 font-semibold">$129.40</span>
            </div>
            <div class="flex justify-between">
              Shipping<span class="text-gray-600 font-semibold">FREE</span>
            </div>
          </div>
          <div class="flex justify-between mt-3 text-lg">
            <p class="text-gray-600 font-semibold">Total</p>
            <span class="text-blue-600 font-semibold text-md">$129.40</span>
          </div>
          <button
            class="bg-[#4E60FF] h-12 text-white rounded-xl border transition-colors border-[#4E60FF] w-full mt-3"
          >
            To'lovni amalga oshirish
          </button>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {};
</script>
<style></style>

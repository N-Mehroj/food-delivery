        ...
        <div class="nav-container justify-between flex pt-6 pb-2 w-full">
          <div class="left-side flex">
            ...
            >
              <input
                type="text"
                class="bg-transparent pl-4 py-1 placeholder:text-[#83859C] text-[#5f606b]"
                placeholder="Search"
              />
              <v-icon
                name="ri-search-line"
                class="text-[#8b8d9c] mr-3 cursor-pointer"
              />
            </div>
          </div>
          <div class="right-side">
            <ul class="menu flex" v-if="isLogedIn">
              <RouterLink
                :to="{ name: 'login' }"
                class="hover:text-[#4E60FF] h-12 hidden md:block"
              ...
              <RouterLink
                :to="{ name: 'login' }"
                class="hover:text-[#4E60FF] h-12 hidden"
            ...
            <ul class="flex menu" v-if="!isLogedIn">
              <RouterLink
                :to="{ name: 'login' }"
                class="hover:text-[#4E60FF] h-12 mr-4"
                >Login</RouterLink
              >
              <RouterLink
                :to="{ name: 'login' }"
